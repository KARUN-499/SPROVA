...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (email != _adminEmail) {
      return const Scaffold(
        backgroundColor: _bg,
        body: Center(
          child: Text('Access denied.', style: TextStyle(color: _txt3)),
        ),
      );
    }
    return Scaffold(
  ...
  Widget _buildTabs() {
    final tabs = ['Exercises', 'Students', 'Submissions', 'Enrollments'];
    return Container(
      decoration: const BoxDecoration(
        border: Border(bottom: BorderSide(color: _line)),
      ),
  ...
  Widget _buildBody() {
    switch (_tab) {
      case 1:
        return _StudentsView(sb: _sb);
      case 2:
        return _SubmissionsView(sb: _sb);
      case 3:
        return _EnrollmentsView(sb: _sb);
      default:
        return _ExercisesView(sb: _sb);
    }
  }
}
...
class _ExercisesViewState extends State<_ExercisesView> {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text(
                    label,
                    style: TextStyle(
                      color: active ? Colors.black : _txt3,
                      fontSize: 12,
                      fontWeight: FontWeight.w700,
                    ),
                  ),
  ...
  void _openForm(BuildContext context, {Map<String, dynamic>? lesson}) =>
      showModalBottomSheet<void>(
        ...
        builder: (_) =>
            _ExerciseForm(sb: widget.sb, lesson: lesson, onSaved: _load),
      );
    ...
    builder: (_) => AlertDialog(
      ...
      actions: [
        TextButton(
          ...
          child: const Text('Cancel', style: TextStyle(color: _txt3)),
        ),
        TextButton(
          onPressed: () async {
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                const _FL('TRACK (empty = all tracks)'),
                const SizedBox(height: 6),
                Container(
                  decoration: BoxDecoration(
                    color: _bg,
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: 'Write the full exercise instructions...',
                      hintStyle: TextStyle(color: _txt4),
                      border: InputBorder.none,
                      contentPadding: EdgeInsets.all(14),
                    ),
...
class _StudentsViewState extends State<_StudentsView> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final enrollments = await widget.sb
        .from('enrollments')
        .select()
        .eq('payment_status', 'completed')
        .order('enrolled_at', ascending: false);
    final submissions = await widget.sb
        .from('submissions')
        .select('user_email, day');
    ...
    if (mounted)
      setState(() {
        _students = List<Map<String, dynamic>>.from(enrollments);
        _subCounts = counts;
        _loading = false;
      });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final atRisk = _students
        .where((s) => (_subCounts[s['user_email']] ?? 0) < 4)
        .toList();
    final onTrack = _students
        .where((s) => (_subCounts[s['user_email']] ?? 0) >= 4)
        .toList();
        ...
        children: [
          ...
          if (atRisk.isNotEmpty) ...[
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 10),
                  Text(
                    '${atRisk.length} student${atRisk.length > 1 ? 's' : ''} falling behind — fewer than 4 submissions this week',
                    style: const TextStyle(
                      color: _red,
                      fontSize: 12,
            ...
            ...atRisk.map(
              (s) => _StudentCard(
                s: s,
                subCount: _subCounts[s['user_email']] ?? 0,
                atRisk: true,
                onViewSubs: () =>
                    _viewSubmissions(context, s['user_email'] as String),
              ),
            ),
            const SizedBox(height: 20),
          ],
  ...
  void _viewSubmissions(BuildContext context, String email) =>
      showModalBottomSheet<void>(
        context: context,
        isScrollControlled: true,
        backgroundColor: Colors.transparent,
...
class _StudentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        color: atRisk ? _redD : _card,
        border: Border.all(color: atRisk ? _red.withOpacity(.3) : _line),
        borderRadius: BorderRadius.circular(10),
      ),
      child: Row(
        children: [
          Container(
            ...
            height: 34,
            decoration: BoxDecoration(
              color: atRisk ? _red.withOpacity(.15) : _greenD,
              shape: BoxShape.circle,
              border: Border.all(
                color: atRisk ? _red.withOpacity(.4) : _green.withOpacity(.3),
              ),
            ),
            child: Center(
              child: Icon(
                atRisk ? Icons.warning_amber_rounded : Icons.check_rounded,
                color: atRisk ? _red : _green,
                size: 16,
              ),
            ),
          ),
...
class _SubmissionsViewState extends State<_SubmissionsView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_subs.isEmpty)
      return const Center(
        child: Text('No submissions yet.', style: TextStyle(color: _txt3)),
      );
    return RefreshIndicator(
      color: _amber,
      backgroundColor: _card,
...
class _EnrollmentsViewState extends State<_EnrollmentsView> {
  ...
  Widget build(BuildContext context) {
    ...
    final completed = _enrollments
        .where((e) => e['payment_status'] == 'completed')
        .length;
    return Column(
      children: [
        Container(
            ...
            child: _enrollments.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final date = e['enrolled_at'] != null
                          ? DateTime.parse(e['enrolled_at'] as String).toLocal()
                          : null;
                      return Container(
                        margin: const EdgeInsets.only(bottom: 8),
                        padding: const EdgeInsets.all(14),
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              height: 34,
                              decoration: BoxDecoration(
                                color: paid ? _greenD : _card2,
                                borderRadius: BorderRadius.circular(8),
                                border: Border.all(
                                  color: paid ? _green.withOpacity(.3) : _line2,
                                ),
                              ),
                              child: Center(
                                child: Icon(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      const Text(
                                        ' · ',
                                        style: TextStyle(color: _txt4),
                                      ),
                                      Text(
                                        'Rs. ${((e['amount_paid'] as int? ?? 0) / 100).round()}',
                                        style: const TextStyle(
                                          color: _txt3,
                                          fontSize: 11,
                                        ),
